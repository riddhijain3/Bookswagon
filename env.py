...
def create_app():
    ...
    app = Flask(__name__)
    app.secret_key = os.getenv('SECRET_KEY', '3dde42c933ad50a0ea052105e5abd23b105a6e07f8cad0f15e3339f6a6648515')
...
def get_connection_string():
    ...
    return f'DRIVER={{{os.getenv("DB_DRIVER")}}};SERVER={os.getenv("DB_SERVER")};DATABASE={os.getenv("DB_DATABASE")};UID={os.getenv("DB_UID")};PWD={os.getenv("DB_PWD")};'

def get_db():
    """Get database connection and cursor from Flask g object or create new ones"""
    if 'db' not in g:
        try:
...
class OrderRepository:
    ...
    def fetch_order_by_id(order_id):
        """Fetch order data from database by order ID"""
        conn, cursor = get_db()
        if not cursor:
            return None
        ...
        try:
            ...
            # Parse results into Order model
            order = Order(
            ...
            # Add books to order
            for row in results:
                if row[2] is not None:  # Product title exists
...
# database/repositories/faq_repository.py
from database.connection import get_db
...
class FaqRepository:
    ...
    def get_faq_knowledge_base():
        ...
        # Build knowledge base from database FAQs
        knowledge_base = "# Bookswagon FAQ Knowledge Base\n\n"
        ...
        # Build knowledge base by category
        for category, category_faqs in categories.items():
            knowledge_base += f"## {category}\n"
            for faq in category_faqs:
                knowledge_base += f"- Question: {faq['question']}\n"
                knowledge_base += f"  Answer: {faq['answer']}\n\n"
...
class AIService:
    ...
    def query_deepseek(messages, temperature=0.1):
        ...
        if not api_key or api_key == "sk-YOUR_DEFAULT_API_KEY_IF_NEEDED":
    ...
    def extract_order_id(text):
        """Extract Bookswagon order ID from text"""
        ...
        try:
            result = AIService.query_deepseek(messages, temperature=0).strip()
    ...
    def detect_language(text):
        ...
        try:
            ...
            result = AIService.query_deepseek(messages, temperature=0).strip().lower()
            return result == "hindi"
        except Exception as e:
    ...
    def get_response_in_language(response, is_hindi):
        ...
        if is_hindi:
            ...
            """
            try:
                return AIService.query_deepseek([{"role": "user", "content": translation_prompt}], temperature=0.3).strip()
            except Exception as e:
    ...
    @staticmethod
    def generate_order_summary(order, user_query, is_hindi):
        """Generate AI response about an order using FAQ knowledge"""
        if not order:
            return AIService.get_response_in_language("I don't have order details to answer that.", is_hindi)

        # Convert order to dict for easier handling
        order_data = order.to_dict()
        order_details = order_data['order_details']
        
        # Create book list
        book_list = ', '.join([f"{i+1}. {book['product_name']}" for i, book in enumerate(order_data['books'])])

        # Get FAQ knowledge base
        faq_knowledge = FaqRepository.get_faq_knowledge_base()
        ...
        Purchased: {order_details.get('purchase_date')}
        Expected Delivery: {order_details.get('promise_date')}
        ...
        return AIService.query_deepseek(messages)

    @staticmethod
    def generate_general_response(chat_history, is_hindi):
        """Generate response for general queries without order context"""
        faq_knowledge = FaqRepository.get_faq_knowledge_base()
...
class FormatterService:
    """Service for formatting order data and responses"""
    ...
    def format_order_response(order):
        ...
        formatted_response = (
            ...
            f"Customer: {order_details.get('customer_name').strip().title() if order_details.get('customer_name') else 'Unknown'}\n"
            f"Purchase Date: {order_details.get('purchase_date')}\n"
        ...
        if order_details.get('order_status', '').lower() != 'cancelled':
            formatted_response += (
                f"Expected Delivery: {order_details.get('promise_date')}\n"
        ...
        formatted_response += (
            f"\nShipping Address:\n"
            f"{order_details.get('shipping_address', '').strip().title() if order_details.get('shipping_address') else 'Unknown'}, "
            f"{order_details.get('shipping_city', '').strip().title() if order_details.get('shipping_city') else ''}, "
            f"{order_details.get('shipping_state', '').strip().title() if order_details.get('shipping_state') else ''} "
            f"{order_details.get('shipping_zip', '').strip() if order_details.get('shipping_zip') else ''}, "
            f"{order_details.get('shipping_country', '').strip().title() if order_details.get('shipping_country') else ''}\n"
    ...
    @staticmethod
    def format_specific_books_response(order, book_indices):
...
def order_middleware(f):
    """Middleware to handle order data"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        # Store active order data in g object for easy access
        g.active_order_id = session.get('active_order_id')
        g.active_order_data = session.get('active_order_data')
...
from api.middleware import session_middleware, order_middleware
...
class ChatController:
    ...
    def handle_message():
        ...
        if any(cmd in user_input.lower() for cmd in EXIT_COMMANDS):
            is_hindi = AIService.detect_language(user_input)
            farewell = AIService.get_response_in_language("Thank you for using Bookswagon support. Have a good day!", is_hindi)
        ...
        # Process message
        response_text = ChatController._process_message(user_input, is_hindi)
